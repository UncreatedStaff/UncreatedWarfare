    ...
    private IEnumerator<WaitForSeconds> TeleportCoroutine(UCPlayer player, object structure, float delay, bool shouldCancelOnMove = false, bool startCoolDown = true, float yawOverride = -1)
    {
        ...
        if (isFOB)
        {
            if (fob!.Bunker != null)
            {
                ...
                ActionLogger.Add(ActionLogType.DEPLOY_TO_LOCATION, "FOB BUNKER " + fob.Name + " TEAM " + Teams.TeamManager.TranslateName(fob.Team, 0), player);
            }
        }
        else if (isCache)
        {
            ...
            ActionLogger.Add(ActionLogType.DEPLOY_TO_LOCATION, "CACHE " + cache.Name + " TEAM " + Teams.TeamManager.TranslateName(cache.Team, 0), player);
        }
        else if (isSpecialFOB)
        {
            position = special!.Position;
            ActionLogger.Add(ActionLogType.DEPLOY_TO_LOCATION, "SPECIAL FOB " + special.Name + " TEAM " + Teams.TeamManager.TranslateName(special.Team, 0), player);
        }
        else if (structure is Vector3 vector)
        {
            position = vector;
            ActionLogger.Add(ActionLogType.DEPLOY_TO_LOCATION, "MAIN BASE " + Teams.TeamManager.TranslateName(player.GetTeam(), 0), player);
        }
