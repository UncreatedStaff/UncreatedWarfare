    ...
    public void Build()
    {
        ...
        foreach (KeyValuePair<ulong, float> entry in PlayerHits)
        {
            ...
            if (contribution >= 0.1F && player != null)
            {
                ...
                ActionLogger.Add(EActionLogType.HELP_BUILD_BUILDABLE, $"{Foundation.asset.itemName} / {Foundation.asset.id} / {Foundation.asset.GUID:N} - {Mathf.RoundToInt(contribution * 100f).ToString(Data.AdminLocale)}%", player);
