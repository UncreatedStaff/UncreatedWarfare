...
public sealed partial class KitSelectionUI : UnturnedUI,
    ...
    IEventListener<PlayerLeft>
{
        ...
        )
    {
        ...
        foreach (KitPanel panel in _panels)
        {
            ...
            ElementPatterns.SubscribeAll(panel.Kits, k => k.UnlockButton, HandleButtonUnlockKitClicked);
        }
    ...
    private bool CanAffordRearmKit(WarfarePlayer player, Kit kit, IAmmoStorage ammoStorage, out float? rearmCost)
    {
        float current = ammoStorage.AmmoCount;
        ...
        if (ammoStorage.AllowDiscountedRearm)
        {
            rearmCost = null;
            return current > 0;
        }

        rearmCost = _rearmService.GetRearmCost(player, kit);
        return current >= rearmCost;
    }
    ...
    public async UniTask UpdateKitAsync(Kit kit, WarfarePlayer? player = null, CancellationToken token = default)
    {
        ...
        bool nitroCheckPanel = false, nitroCheckList = false;
        // public kits
        ...
        if (!nitroCheckList && !nitroCheckPanel)
        {
            return;
        }
    ...
    private async Task<RequestKitResult> RequestKitAsync(WarfarePlayer player, KitSelectionUIData data, Kit kit, int index, Class @class, bool isFavorite)
    {
        ...
        try
        {
            if (data.AmmoStorage != null)
            {
                ...
                semaphore = data.AmmoStorage.InteractSemaphore;
                if (semaphore != null)
                {
                ...
                if (ammoCost > 0 && (data.AmmoStorage.AllowDiscountedRearm
                        ? data.AmmoStorage.AmmoCount <= 0
                        : data.AmmoStorage.AmmoCount < ammoCost))
                {
                    if (isFavorite)
                        SendFavoriteKit(index, kit, player, data, false);
                    else
                        UpdateStatusLabels(GetKitInfoUI(@class, index), false, data, @class, index, player, kit, player.Component<KitPlayerComponent>());
                    return new RequestKitResult { Success = false };
                }
            }
            ...
            {
                await UniTask.SwitchToMainThread();
                data.AmmoStorage.SubtractAmmo(ammoCost);
    ...
    void IEventListener<SquadUpdated>.HandleEvent(SquadUpdated e, IServiceProvider serviceProvider)
    {
        ...
        {
            UpdateAllKitsStatus(player, false);
        }
    }
    ...
    private void OnPointsChanged(WarfarePlayer player, double deltaXp, double deltaCredits, double deltaReputation)
    {
        ...
        UpdateAllKitsStatus(
            player,
            true,
            (in info) => info.LabelState is StatusState.CreditCost || info.ButtonState == PurchaseButtonState.CreditCost
        );
    }

    private delegate bool ShouldUpdateKit(in KitCacheInformation info);
    private void UpdateAllKitsStatus(WarfarePlayer player, bool force, ShouldUpdateKit? filter = null)
    {
        KitSelectionUIData? data = GetData<KitSelectionUIData>(player.Steam64);
        if (data is not { HasUI: true })
            return;
        ...
        {
            KitPanel panel = _panels[i];
            for (int j = 0; j < panel.Kits.Length; ++j)
            {
                Class @class = GetPanelClass(i);
                ref KitCacheInformation info = ref data.GetCachedState(@class, j);
                if (info.Kit == null)
                    break;

                if (filter != null && !filter(in info))
                    continue;

                UpdateActionButtons(info.Kit, player, panel.Kits[j], data, j, @class);
                UpdateStatusLabels(panel.Kits[j], false, data, @class, j, player, info.Kit, kpc, force);
            }
        }
        ...
        for (int i = 0; i < _listResults.Length; ++i)
        {
            ...
            UpdateActionButtons(info.Kit, player, _listResults[i], data, i);
            UpdateStatusLabels(_listResults[i], false, data, Class.None, i, player, info.Kit, kpc, force);
        }
    }

    private async UniTask TempUncloseAsync(WarfarePlayer player, CancellationToken token = default)
    {
        await UniTask.SwitchToMainThread(token);
    ...
    private void UpdateRestockShortcut(KitSelectionUIData data, WarfarePlayer player, float? currentRearmCost, bool fromDefaultValues)
    {
        ...
        else
        {
            ...
            else if (float.IsFinite(ammoCount))
            {
                ...
                if (data.AmmoStorage.AllowDiscountedRearm ? ammoCount > 0 : ammoCount >= rearmCost)
                {
                ...
                {
                    _restockShortcutButton.Label.SetText(player, _translations.RestockShortcutButtonNoAmmo.Translate(player));
                }
            }
            else
            {
            ...
            if (!fromDefaultValues || !enabled)
                _restockShortcutButton.SetState(player, enabled);
        }
    }
    ...
    private void SetListOpened(KitSelectionUIData data, WarfarePlayer player, bool isList)
    {
        if (data.IsListOpen == isList)
            return;

        UnturnedUIElement element = isList ? _switchToListLogic : _switchToPanelLogic;
    ...
    private void SendKitInfo(KitInfo ui, WarfarePlayer player, Kit kit, KitPlayerComponent kitAccessComp, KitSelectionUIData data, bool fromDefaultValues, int index, Class @class = Class.None)
    {
        ...
        UpdateActionButtons(kit, player, ui, data, index, @class, fromDefaultValues);
    ...
    private void UpdateStatusLabels(KitInfo ui, bool fromDefaultValues, KitSelectionUIData data, Class @class, int index, WarfarePlayer player, Kit kit, KitPlayerComponent kitAccessComp, bool force = true)
    {
        ...
        if (activeKit != null && activeKit.Key == kit.Key)
        {
            ...
            if (doRearm)
            {
                ...
                if (rearmCost > 0)
                {
                    ...
                    else if (data.AmmoStorage.AllowDiscountedRearm || data.AmmoStorage.AmmoCount >= rearmCost)
                    {
                    ...
                    {
                        defaultStatus = false;
                        ui.StatusLabel.SetText(c, _translations.StatusNoAmmo.Translate(player));
        ...
        ref KitCacheInformation info = ref data.GetCachedState(in state);
        PurchaseButtonState btnState = info.ButtonState;
        StatusState lblState = info.LabelState;
        ...
        bool statusLblDefault = true, showUnlockSection = true;
        if (data.AmmoStorage != null)
        {
            ...
            else
            {
                ...
                {
                    _ = kit.Items;
                    if (CanAffordRearmKit(player, kit, data.AmmoStorage, out float? ammoCost))
                    {
                        float ammoCount = data.AmmoStorage.AmmoCount;
                        if (float.IsFinite(ammoCount))
                        {
                            ammoCost ??= _rearmService.GetRearmCost(player, kit);
                            float ammoCostRnd = MathF.Round(ammoCost.Value, 1);
                            float left = MathF.Round(Math.Max(0, ammoCount - ammoCost.Value), 1);
                            ui.UnlockButton.SetText(c, _translations.PurchaseButtonRequestWithAmmoCost.Translate(ammoCostRnd, left, player));
                        }
                        else
                        {
        ...
        ui.UnlockSection.SetVisibility(c, showUnlockSection);
    ...
    private class KitSelectionUIData : IUnturnedUIData
    {
        ...
        public bool Matches(in KitRequirementsState state, PurchaseButtonState buttonState, StatusState labelState)
        {
            ref KitCacheInformation info = ref GetCachedState(in state);
            return info.ButtonState == buttonState && info.LabelState == labelState;
        }
        ...
        internal void HandleAmmoCountUpdated(float newAmount)
        {
    ...
    private class KitRequirementVisitor(KitSelectionUI @this) : IKitRequirementVisitor<KitRequirementsState>
    {
        ...
        public void AcceptRequiresSquadNotMet(in KitRequirementResolutionContext<KitRequirementsState> ctx, bool needsSquadLead)
        {
            ...
            if (squad == null && _this._squadManager != null && (!needsSquadLead || _this._squadManager.CanCreateNewSquad(ctx.Team)))
            {
                ...
                if (_this._squadMenu == null)
                {
                    if (!ctx.State.FromDefaults)
                        ctx.State.UI.UnlockSection.Hide(ctx.State.Connection);
                    button = PurchaseButtonState.None;
                }
                else
                {
                    ctx.State.UI.UnlockSection.Show(ctx.State.Connection);
...
public sealed class KitSelectionUITranslations : TranslationCollection
{
    ...
    [TranslationData("Label for the purchase button shown when the kit needs to be bought with real money.")]
    ...
    [TranslationData("Label for the URL request when clicking the purchase button for a kit that needs to be bought with real money.")]
