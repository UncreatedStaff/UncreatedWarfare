            ...
            if (CommandWindow.shouldLogChat)
            {
                ...
                switch (mode)
                {
                    case EChatMode.GLOBAL:
                        ...
                        if (ShouldLog(text))
                            ActionLogger.Add(ActionLogType.CHAT_GLOBAL, text, callingPlayer.playerID.steamID.m_SteamID);
                        break;
                    case EChatMode.LOCAL:
                        L.Log($"[A/S]  {name} \"{text}\"", ConsoleColor.DarkGray);
                        if (ShouldLog(text))
                            ActionLogger.Add(ActionLogType.CHAT_AREA_OR_SQUAD, text, callingPlayer.playerID.steamID.m_SteamID);
                        break;
                    case EChatMode.GROUP:
                        L.Log($"[TEAM] {name} \"{text}\"", ConsoleColor.DarkGray);
                        if (ShouldLog(text))
                            ActionLogger.Add(ActionLogType.CHAT_GROUP, text, callingPlayer.playerID.steamID.m_SteamID);
                        break;
                    default:
                        return false;
                }
            ...
            if (caller != null)
            {
                ...
                if (!duty)
                {
                    ...
                    if (match.Success && match.Length > 0)
                    {
                        ...
                        ActionLogger.Add(ActionLogType.CHAT_FILTER_VIOLATION, mode switch { EChatMode.LOCAL => "AREA/SQUAD: ", EChatMode.GLOBAL => "GLOBAL: ", _ => "TEAM: " } + text, caller);
                        return false;
                    }
                }
            }
