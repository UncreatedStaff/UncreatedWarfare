...
public class VehicleBay : ListSqlSingleton<VehicleData>, ILevelStartListenerAsync, IDeclareWinListenerAsync, IPlayerPostInitListenerAsync, IQuestCompletedHandler
{
    ...
    Task IPlayerPostInitListenerAsync.OnPostPlayerInit(UCPlayer player, bool wasAlreadyOnline, CancellationToken token)
    {
    ...
    protected override async Task AddOrUpdateItem(VehicleData? item, PrimaryKey pk, CancellationToken token = default)
    {
        ...
        await Sql.QueryAsync(
            F.BuildInitialInsertQuery(TABLE_MAIN, COLUMN_PK, hasPk, COLUMN_EXT_PK,
                new string[] { TABLE_DELAYS, TABLE_ITEMS, TABLE_CREW_SEATS, TABLE_UNLOCK_REQUIREMENTS, TABLE_BARRICADES, TABLE_TRUNK_ITEMS },
