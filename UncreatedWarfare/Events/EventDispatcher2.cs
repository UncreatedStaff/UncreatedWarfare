...
/// </summary>
public partial class EventDispatcher2 : IHostedService, IDisposable
{
    ...
    private static readonly MethodInfo InvokeAsyncOpenGenericMtd = typeof(EventDispatcher2).GetMethod(nameof(InvokeAsyncOpenGeneric), BindingFlags.NonPublic | BindingFlags.Instance)!;
    private static readonly MethodInfo InvokeNormalOpenGenericMtd = typeof(EventDispatcher2).GetMethod(nameof(InvokeNormalOpenGeneric), BindingFlags.NonPublic | BindingFlags.Instance)!;
    ...
    private void FindEventListenerProviders(ILifetimeScope scope, List<IEventListenerProvider> providers)
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            foreach (IComponentRegistration serviceRegistration in compReg.Registrations)
            {
                ...
                if (!InScope(childScope, serviceRegistration))
                    continue;
