...
public class FobDestroyed : IActionLoggableEvent, IFobNeedsUIUpdateEvent
{
    ...
    public ActionLogEntry GetActionLogEntry(IServiceProvider serviceProvider, ref ActionLogEntry[]? multipleEntries)
    {
        return new ActionLogEntry(ActionLogTypes.FobDestroyed,
            ...
            $"Damagers: [ {(Fob is IDamageableFob f ? string.Join(", ", f.DamageTracker.Contributors.Select(x => $"{x.m_SteamID} - {f.DamageTracker.GetDamageContributionPercentage(x, false):P2}")) : "?")} ]",
            Event.InstigatorId
        );
    }
