...
public partial class EventDispatcher : IHostedService, IDisposable
{
    ...
    UniTask IHostedService.StartAsync(CancellationToken token)
    {
        ...
        _playerService.SubscribeToPlayerEvent<PlayerEquipRequestHandler>((player, value) => player.equipment.onEquipRequested += value, OnPlayerEquipRequested);
        _playerService.SubscribeToPlayerEvent<PlayerDequipRequestHandler>((player, value) => player.equipment.onDequipRequested += value, OnPlayerDequipRequested);
    ...
    async UniTask IHostedService.StopAsync(CancellationToken token)
    {
        ...
        PlayerLife.OnSelectingRespawnPoint -= OnPlayerChooseSpawnAfterDeath;
        PlayerLife.OnPreDeath -= PlayerLifeOnOnPreDeath;
        _playerService.UnsubscribeFromPlayerEvent<PlayerEquipRequestHandler>((player, value) => player.equipment.onEquipRequested -= value, OnPlayerEquipRequested);
        _playerService.UnsubscribeFromPlayerEvent<PlayerDequipRequestHandler>((player, value) => player.equipment.onDequipRequested -= value, OnPlayerDequipRequested);
