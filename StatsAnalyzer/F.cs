    ...
    public static class F
    {
        public static string GetTimeFromMinutes(this uint minutes)
        {
            ...
            else if (minutes < 1440) // < 1 day 
            {
                ...
                return $"{hours} hr{hours.S()}{(minutesOverflow == 0 ? "" : $" & {minutesOverflow} min{minutesOverflow.S()}")}";
            }
            else if (minutes < 43800) // < 1 month (30.416 days)
            {
                ...
                return $"{days} day{days.S()}{(hoursOverflow == 0 ? "" : $" & {hoursOverflow} hr{hoursOverflow.S()}")}";
            }
            else if (minutes < 525600) // < 1 year
            {
                ...
                return $"{months} mon{months.S()}{(daysOverflow == 0 ? "" : $" & {daysOverflow} day{daysOverflow.S()}")}";
            }
            else // > 1 year
            {
                ...
                return $"{years} yr{years.S()}{(monthOverflow == 0 ? "" : $" & {monthOverflow} mon{monthOverflow.S()}")}";
            }
        }
